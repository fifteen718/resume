	...
	update: function(html, loadScripts, callback) {
		...
		Ext.lib.Event.onAvailable(id, function() {
			var DOC = document,
				...
				srcRe = /\ssrc=([\'\"])(.*?)\1/i,
				typeRe = /\stype=([\'\"])(.*?)\1/i,
				match, attrs, srcMatch, typeMatch, el, s;
...
Ext.Element.addMethods(function() {
	var h = {},
		...
		u = Ext.isIE ? "styleFloat" : "cssFloat",
		D = /alpha\(opacity=(.*)\)/i,
		l = /^\s+|\s+$/g,
		B = Ext.Element,
		d = "padding",
...
Ext.Element.addMethods(function() {
	...
	return {
		...
		anim: function(p, q, n, s, o, m) {
			...
			q = q || {};
			var r = this,
				t = Ext.lib.Anim[n](r.dom, p, (q.duration || s) || 0.35, (q.easing || o) || "easeOut", function() {
					if (m) {
						m.call(r)
					}
					if (q.callback) {
	...
	readRecords: function(r) {
		...
		for (var z = 0, B = u.length; z < B; z++) {
			...
			for (var y = 0, m = q.length; y < m; y++) {
				var C = q.items[y],
					x = C.mapping !== undefined && C.mapping !== null ? C.mapping : y;
...
Ext.MessageBox = function() {
	...
	return {
		...
		updateText: function(B) {
			...
			l.update(B || "&#160;");
			var z = d != "" ? (j.getWidth() + j.getMargins("lr")) : 0,
				D = l.getWidth() + l.getMargins("lr"),
				A = u.getFrameWidth("lr"),
				C = u.body.getFrameWidth("lr"),
				y;
			if (Ext.isIE && z > 0) {
				z += 3
			}
...
Ext.TabPanel = Ext.extend(Ext.Panel, {
	...
	onAdd: function(b) {
		...
		if (this.rendered) {
			...
			if (a.getCount() == 1 && !this.collapsed) {
				this.syncSize()
			}
...
var swfobject = function() {
	var F = "undefined",
		...
		O = function() {
			var ac = typeof k.getElementById != F && typeof k.getElementsByTagName != F && typeof k.createElement != F,
				aj = u.userAgent.toLowerCase(),
				aa = u.platform.toLowerCase(),
				...
				ai = [0, 0, 0],
				ad = null;
			if (typeof u.plugins != F && typeof u.plugins[U] == s) {
				ad = u.plugins[U].description;
				if (ad && !(typeof u.mimeTypes != F && u.mimeTypes[r] && !u.mimeTypes[r].enabledPlugin)) {
					V = true;
					Z = false;
		...
		if (ai > 0) {
			for (var ah = 0; ah < ai; ah++) {
				...
				if (O.pv[0] > 0) {
					var ag = c(aa);
					if (ag) {
						if (H(p[ah].swfVersion) && !(O.wk && O.wk < 312)) {
							y(aa, true);
							if (ad) {
								ac.success = true;
								ac.ref = B(aa);
							...
							if (p[ah].expressInstall && C()) {
								var ak = {};
								ak.data = p[ah].expressInstall;
	...
	function B(ac) {
		...
		if (aa && aa.nodeName == "OBJECT") {
			if (typeof aa.SetVariable != F) {
				...
				var ab = aa.getElementsByTagName(s)[0];
				if (ab) {
					Z = ab
				}
			}
	...
	function R(ac, ad, Z, ab) {
		...
		if (ag) {
			...
			var af = O.ie && O.win ? "ActiveX" : "PlugIn",
				ae = "MMredirectURL=" + Q.location.toString().replace(/&/g, "%26") + "&MMplayerType=" + af + "&MMdoctitle=" + k.title;
	...
	function h(ae) {
		...
		if (O.win && O.ie) {
			...
			var aa = ae.getElementsByTagName(s)[0];
			if (aa) {
	...
	function x(ae, aa, af, ad) {
		...
		if (O.ie && O.win) {
			...
			if (o && typeof k.createTextNode != F) {
				o.appendChild(k.createTextNode(ae + " {" + aa + "}"))
			}
		}
	}
...
Ext.grid.GridView.SplitDragZone = Ext.extend(Ext.dd.DDProxy, {
	...
	handleMouseDown: function(g) {
		var c = this.view.findHeaderCell(g.getTarget());
		if (c && this.allowHeaderDrag(g)) {
			var d = this.view.fly(c).getXY(),
				a = d[0],
				h = d[1];
			exy = g.getXY(), ex = exy[0], w = c.offsetWidth, adjust = false;
			if ((ex - a) <= this.hw) {
				adjust = -1
			} else {
				if ((a + w) - ex <= this.hw) {
			...
			if (adjust !== false) {
				...
				var b = this.view.getCellIndex(c);
				if (adjust == -1) {
					if (b + adjust < 0) {
						return
					}
					while (this.cm.isHidden(b + adjust)) {
						--adjust;
						if (b + adjust < 0) {
							return
						}
				...
				if (this.view.columnDrag) {
					this.view.columnDrag.callHandleMouseDown(g)
				}
			}
		}
	},
...
Ext.grid.HeaderDropZone = Ext.extend(Ext.dd.DropZone, {
	...
	positionIndicator: function(d, k, j) {
		var a = Ext.lib.Event.getPageX(j),
			...
			c, i, b = g.top + this.proxyOffsets[1];
...
Ext.grid.GroupingView = Ext.extend(Ext.grid.GridView, {
	...
	doRender: function(c, h, q, a, p, s) {
		...
		var A = this.getGroupField(),
			o = this.cm.findColumnIndex(A),
			x, j = "width:" + this.getTotalWidth() + ";",
			e = this.cm.config[o],
		...
		for (u = 0, v = h.length; u < v; u++) {
			var k = a + u,
				m = h[u],
				d = m.data[A];
			x = this.getGroup(d, m, b, k, o, q);
			if (!l || l.group != x) {
				n = this.constructId(d, A, o);
		...
		var y = [];
		for (u = 0, v = z.length; u < v; u++) {
			x = z[u];
			this.doGroupStart(y, x, c, q, p);
			y[y.length] = Ext.grid.GroupingView.superclass.doRender.call(this, c, x.rs, q, x.startRow, p, s);
			this.doGroupEnd(y, x, c, q, p)
		}
		return y.join("")
	},
